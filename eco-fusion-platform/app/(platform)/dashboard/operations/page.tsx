            ...
            {zones.map(zone => (
                <div key={zone.id} className="space-y-6">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                        ...
                        {zone.metrics.do && <SensorWidget label="Dissolved O2" value={zone.metrics.do.toString()} unit="mg/L" status={zone.metrics.do < 6 ? "warning" : "good"} />}
                        {zone.metrics.ammonia !== undefined && <SensorWidget label="Ammonia" value={zone.metrics.ammonia.toString()} unit="ppm" status={zone.metrics.ammonia > 0.05 ? "warning" : "good"} />}
                        {zone.metrics.humidity && <SensorWidget label="Humidity" value={zone.metrics.humidity.toString()} unit="%" status={zone.metrics.humidity < 40 ? "warning" : "good"} />}
                        {/* Fallback for generic sensors */}
